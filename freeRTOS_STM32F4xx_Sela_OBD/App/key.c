...
void DispErrCode(short val) 
{
	...
	if(DVal[0]<10 && DVal[1]<10 && DVal[2]<10 && DVal[3]<10)
	  Disp(ch_num[DVal[0]], ch_num[DVal[1]], ch_num[DVal[2]], ch_num[DVal[3]], ch_E);
}

/*
********************************************************************************
* Description : Analysis_Reset
* Arguments   : None
* Return      : None
* Note        : 
******************************************************************************** 
...
void Analysis_Reset(void) 
{
  u16 addr;
	...
	Setting.PAlarm.PLAlarmFlag = 0;
	Setting.PAlarm.PHAlarmFlag = 0;
  addr = DRV_SETTING_PARM + offsetof(st_Setting, PAlarm); 
  FRAMMultiWrite((u8 *)&Setting.PAlarm, addr, sizeof(Setting.PAlarm));

	HPHCnt = 0;
	Flags1.HPHCntFlag =0;
	HPLCnt = 0;
	Flags1.HPLCntFlag =0;
  LPRESS_LED(Setting.PAlarm.PLAlarmFlag);
  if(!Flags1.TLvHAlarmFlag & !Flags1.TLvLAlarmFlag) NORMAL_LED(ON);
}
  ...
  u16 addr;
	...
	if(keyflag.hmode == 0) {
		...
		if(!Flags.PasswdFlag) 
			keyflag.hmode = 0;
		if(!keyflag.setcnt)
			Disp(ch_ba, ch_R, ch_E, ch_S, ch_U);		// Display 'USER-'
		else if(keyflag.setcnt==1){
			DispVal(ReadValue.User);
		}	
		else if(keyflag.setcnt>1 && keyflag.setcnt<6) { 
      ...
      ReadValue.User = DVal[3]*1000+DVal[2]*100+DVal[1]*10+DVal[0];
      Setting.UserNumber = ReadValue.User;
...
void Display_Parm(void) 
{
  if(Flags1.DispFlag){
    if(keyflag.mode == 0) 
    {
      if(Flags1.TLvHAlarmFlag || Flags1.TLvLAlarmFlag)    
         Disp(ch_O, ch_L, ch_ba, ch_L, ch_U);          // UL-LO 
      else if(Disk_Full)    
         Disp(ch_L, ch_U, ch_F, ch_ba, ch_S);          // S-FUL            
      else
         DispVal(ReadValue.UreaLevel);		  	         // Urea Level
    }	    
      ...
      if(!keyflag.setcnt)
        Disp(ch_E, ch_T, ch_A, ch_T, ch_S);					// Display 'State'
      else {
        DispVal(ReadValue.State);
      }
    }
...
/*
********************************************************************************
* Description : Hidden Mode process(Initialize)
* Arguments   : None
* Return      : None
* Note        : 
******************************************************************************** 
...
void Calibration(void) 
{
  u16 addr;
	...
	else if(keyflag.cmode == 1)
		Disp(ch_num[8], ch_num[8], ch_num[8], ch_num[8], ch_num[8]);  // Display '8888'
  else if(keyflag.cmode == 2)
		Disp(ch_ba, ch_ba, ch_L, ch_A, ch_C);		// Display 'CAL-'
	else if(keyflag.cmode == 3) {
//    DefaultSet();
    Setting.T1Offset = 0;
    keyflag.cmode = 4;
	}
  else if(keyflag.cmode == 4)
			Disp(ch_ba, ch_ba, ch_ba, ch_I, ch_H);		// Display 'HI-'
  else if(keyflag.cmode == 5)
			DispVal(ReadValue.DpfPress);
  else if(keyflag.cmode == 6) {
 		if(ReadValue.DpfPress > 500 && ReadValue.DpfTemp > 400) {
      TSum = ReadValue.DpfTemp - TCal;
      PHCal = adcdata[ADC_AVR_P0];
      
      Setting.P1CalHigh = PHCal;
      addr = DRV_SETTING_PARM + offsetof(st_Setting, P1CalHigh); 
      FRAMMultiWrite((u8 *)&Setting.P1CalHigh, addr, sizeof(Setting.P1CalHigh));
  ...
  else if(keyflag.cmode == 7)
			Disp(ch_ba, ch_T, ch_E, ch_S, ch_H);		// Display 'HSET-'
  else if(keyflag.cmode == 8)
			Disp(ch_ba, ch_ba, ch_ba, ch_O, ch_L);		// Display 'LO-'
  else if(keyflag.cmode == 9)
			DispVal(ReadValue.DpfPress);
  else if(keyflag.cmode == 10) {
 		if(ReadValue.DpfPress < 600 && ReadValue.DpfTemp > 400) {
      TSum += ReadValue.DpfTemp - TCal;
      TOffset = TSum / 2;
      PLCal = adcdata[ADC_AVR_P0];
      
      Setting.P1CalLow = PLCal;
      addr = DRV_SETTING_PARM + offsetof(st_Setting, P1CalLow); 
      FRAMMultiWrite((u8 *)&Setting.P1CalLow, addr, sizeof(Setting.P1CalLow));
      
      Setting.T1Offset = TOffset;
      addr = DRV_SETTING_PARM + offsetof(st_Setting, T1Offset); 
      FRAMMultiWrite((u8 *)&Setting.T1Offset, addr, sizeof(Setting.T1Offset));
			keyflag.cmode = 11;
		}
  ...
  else if(keyflag.cmode == 11) {
		Disp(ch_ba, ch_T, ch_E, ch_S, ch_L);		// Display 'LSET-'   
  }
}
