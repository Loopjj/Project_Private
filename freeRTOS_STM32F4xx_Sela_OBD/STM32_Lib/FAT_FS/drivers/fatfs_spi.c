...
#define SPI_BaudRatePrescaler_SPI_SD  SPI_BaudRatePrescaler_32	//SPI_BaudRatePrescaler_8
...
{
	TmrStop(TMR_SDINTERVAL);
}
void TM_DELAY_SetTime(uint32_t interval)
{
	TmrStop(TMR_SDINTERVAL);
	TmrSetT(TMR_SDINTERVAL, interval);
	TmrStart(TMR_SDINTERVAL);
}
uint32_t TM_DELAY_Time(void)
{
	return TmrChk(TMR_SDINTERVAL);
}
...
{
	//printk("rcvr_spi_multi: inside");
	
	TM_SPI_ReadMulti(FATFS_SPI, buff, 0xFF, btr);
	
	//printk("rcvr_spi_multi: done"); 
}
...
)
{
	...
	do {
		d = xchg_spi(0xFF);
	} while (d != 0xFF && TM_DELAY_Time());	/* Wait for card goes ready or timeout */
	if (d == 0xFF) {
		printk("wait_ready: OK\n");
	} else {
...
)
{
	...
	printk("rcvr_datablock: inside\n");
	
	//Timer1 = 200;
	
	TM_DELAY_SetTime(200);
	do {							// Wait for DataStart token in timeout of 200ms 
		token = xchg_spi(0xFF);
		// This loop will take a time. Insert rot_rdq() here for multitask envilonment. 
	} while ((token == 0xFF) && TM_DELAY_Time());
	if (token != 0xFE) {
		printk("rcvr_datablock: token != 0xFE\n");
		return 0;		// Function fails if invalid DataStart token or timeout 
	}

	rcvr_spi_multi(buff, btr);		// Store trailing data to the buffer 
	xchg_spi(0xFF); xchg_spi(0xFF);			// Discard CRC 
	printk("rcvr_datablock: return = 1\n");
...
)
{
	...
	printk("xmit_datablock: inside\n");

	if (!wait_ready(500)) {
		printk("xmit_datablock: not ready\n");
		return 0;		/* Wait for card ready */
	}
	printk("xmit_datablock: ready\n");
...
)
{
	...
	printk("send_cmd = [0x%02X]: inside\n",cmd);
	
	if (cmd & 0x80) {	/* Send a CMD55 prior to ACMD<n> */
		printk("send_cmd: 0x80 bit set\n");
		cmd &= 0x7F;
	...
	if (cmd == CMD12) {
		printk("send_cmd: CMD12, receive command resp\n");
...
uint8_t TM_FATFS_Detect(void) {
	...
	//return !GPIO_ReadInputDataBit(FATFS_USE_DETECT_PIN_PORT, FATFS_USE_DETECT_PIN_PIN);
	return !SDCARD_DETECT();
#else
	return 1;
#endif
}
...
DSTATUS TM_FATFS_SD_disk_initialize (void) {
	...
	if (send_cmd(CMD0, 0) == 1) {				/* Put the card SPI/Idle state */
		printk("disk_initialize: CMD0 = 1\n");
		//Timer1 = 1000;						/* Initialization timeout = 1 sec */
		TM_DELAY_SetTime(1000);
		if (send_cmd(CMD8, 0x1AA) == 1) {	/* SDv2? */
			for (n = 0; n < 4; n++) ocr[n] = xchg_spi(0xFF);	/* Get 32 bit return value of R7 resp */
			if (ocr[2] == 0x01 && ocr[3] == 0xAA) {				/* Is the card supports vcc of 2.7-3.6V? */
				while (TM_DELAY_Time() && send_cmd(ACMD41, 1UL << 30)) ;	/* Wait for end of initialization with ACMD41(HCS) */
				if (TM_DELAY_Time() && send_cmd(CMD58, 0) == 0) {		/* Check CCS bit in the OCR */
					for (n = 0; n < 4; n++) {
						ocr[n] = xchg_spi(0xFF);
					}
			...
			while (TM_DELAY_Time() && send_cmd(cmd, 0));			/* Wait for end of initialization */
			if (TM_DELAY_Time() || send_cmd(CMD16, 512) != 0) {	/* Set block length: 512 */
				ty = 0;
			}
		}
	}
	TM_FATFS_SD_CardType = ty;	/* Card type */
	printk("disk_initialize: deselecting\n");
	deselect();
...
{
	printk("disk_write: inside\n");
	if (!TM_FATFS_Detect()) {
		return RES_ERROR;
	}
	if (!TM_FATFS_WriteEnabled()) {
		printk("disk_write: Write protected!!! \n---------------------------------------------\n");
		return RES_WRPRT;
	}
