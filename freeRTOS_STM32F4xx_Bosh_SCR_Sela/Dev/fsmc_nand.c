...
void Nand_Read_BadblockData(void)
{
	...
	if(i == 10) {
		printd("===Bad block table not found===\n");
		return;
	}
...
INT16U CMD_functionTest(void)
{
#if 0
	INT16U page,block,zone;
  
	if(CLI_GetArgCnt() < 3) return INV_PRM;
	page = comparison_num((char *)P_ARG(1));
	block = comparison_num((char *)P_ARG(2));
	zone = comparison_num((char *)P_ARG(3));
