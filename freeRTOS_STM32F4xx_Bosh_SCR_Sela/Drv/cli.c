...
__INLINE void dput_ch_console(INT8U data)
{
	while(USART_GetFlagStatus(UART4,USART_FLAG_TXE) == RESET);// OS_Sched();
	USART_SendData(UART4,data);
}

__INLINE void Serial_Com3_Send(INT8U data)
{
	while(USART_GetFlagStatus(USART3,USART_FLAG_TXE) == RESET);// OS_Sched();
	USART_SendData(USART3,data);
}
void Com3PutCharStr(int port,INT8U *data,int len)
{
	int i=0;
	for(i=0;i< len; i++)
		Serial_Com3_Send(data[i]);
}
