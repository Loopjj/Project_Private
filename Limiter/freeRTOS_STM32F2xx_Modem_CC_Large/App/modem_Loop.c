...
void Modem_RxDecode_Loop(void)
{
  ...
  //          SPI_FLASH_BufferWrite(Gpck.Data, naddr, nsize);
    ...
    case CM_F2IF: 
      memcpy(&F2_Info, Gpck.Data, sizeof(tsFWIF));
      printk("\n##### DownLoad Command #####\n");
      printk("File size          : %dBytes\n",F2_Info.Size);
      printk("One Packet size    : %dBytes\n",F2_Info.DivisionSize);
      printk("Download Code Check: 0x%04x\n",F2_Info.CRC16);      
      ...
      if(F2_Info.Type == 0){      
        if(SPI_FLASH_BufferWrite((INT8U *)&tPrjcode, PROJECT_BASE, sizeof(Prjcode_t))) 
        {
          /*-------------------------------------------------------------------------------*/	
          // 다운로드 파일 사이즈 만큼의 SPI Flash의 영역을 지운다.
          sectCnt = (F2_Info.Size / SPI_FLASH_BLOCK)+1;
          if(sectCnt > FLASH_DOWN_BLOCKCOUNT) sectCnt = FLASH_DOWN_BLOCKCOUNT;
          ...
          /*-------------------------------------------------------------------------------*/	
          tPrjcode.file_size = F2_Info.Size;
        }
    
        sd[0] = 1;
        Flags.SecIntFlag = 0;
        Modem_TxData_Loop(CM_F2IF, Gpck.Header.Index, Gpck.Header.Count, 1, sd);
        printk("\nMODEM : Firmware Info Recieve Ok\n");      
        DataSvrWaitCount = 0;
      }
      break;
    ...
    case CM_F2SD: 
      {
        u32 naddr;        
        if(F2_Info.Type == 0) {        
          naddr = ECUDATA_SECTOR + (Gpck.Header.Index * F2_Info.DivisionSize);       
          u16 nsize = F2_Info.DivisionSize;
          if(Gpck.Header.Index < Gpck.Header.Count) {          
            SPI_FLASH_BufferWrite(Gpck.Data, naddr, nsize);
          }
          else {
            crc_same_flag = 0;
            u16 ccrc=0;
            nsize = F2_Info.Size % F2_Info.DivisionSize;
            for(int i = 0; i < Gpck.Header.Count; i++) {
              naddr = ECUDATA_SECTOR + (i * F2_Info.DivisionSize);
              
              if(i < (Gpck.Header.Count - 1)) { 
                nsize = F2_Info.DivisionSize; 
                SPI_FLASH_BufferRead(ModemBuff, naddr, nsize);
              }
              else {
                nsize = F2_Info.Size % F2_Info.DivisionSize; 
            ...
            if(ccrc == F2_Info.CRC16) {
              // 마지막 계산된 CRC 값 저장 -> PROJECT_BASE 번지 
              tPrjcode.file_crc = ccrc;

              // Flash에 저장된 데이터에 대한 CRC 값 Display
              ramCRC = GetFlashAreaCRC(ECUDATA_SECTOR, F2_Info.Size);
              printk("Check wrote flash fileCRC: [%04X]\n", (INT16U)ramCRC);

              SPI_FLASH_BufferWrite((INT8U *)&tPrjcode, PROJECT_BASE, sizeof(Prjcode_t));		
              Setting.ECUFW_CRC16 = ramCRC;			
              crc_same_flag = 1;
            }
          }  
          sd[0] = 1;
          Modem_TxData_Loop(CM_F2SD, Gpck.Header.Index, Gpck.Header.Count, 1, sd);
          printk("\nMODEM : Firmware Recieve %u / %u\n", Gpck.Header.Index, Gpck.Header.Count);        
          DataSvrWaitCount = 0;
        }
      }
    ...
    case CM_F2AK:
      if(F2_Info.Type == 0) {      
        Modem_TxData_Loop(CM_F2AK, Gpck.Header.Index, Gpck.Header.Count, 1, (u8*)&crc_same_flag); 
        Modem_SocketClose(0);
        if(crc_same_flag) {
          Setting.ECUFW_Size = F2_Info.Size;
          Setting.ECUFW_flag = 1;  
          addr = DRV_SETTING_PARM + offsetof(st_Setting, ECUFW_Size);
          FRAMMultiWrite((u8 *)&Setting.ECUFW_Size, addr, 7);
//          keyflag.hidden=3;
//          keyflag.setcnt=2;
        }        
        else {
    ...
    case CM_P1DR:
      if(!ServerRW) {
        ...
        memset(&ScrMap,0,sizeof(stScrMap));
        ServerRW = 0x01;
      }
      break;
    case CM_P1DD:
      if(!ServerRW) {
        ...
        DataSvrStep = DATA_SVR_RECIEVE_WAIT;
        memset(&ScrMap,0,sizeof(stScrMap));
        memcpy(&ScrMap, Gpck.Data, sizeof(stScrMap));
        ServerRW = 0x11;
      }
      break; 
    case CM_P2DR:
      if(!ServerRW) {
        DataSvrWaitCount = 0;
        DataSvrStep = DATA_SVR_RECIEVE_WAIT;
        memset(&SysCtl,0,sizeof(stSysControl));
        ServerRW = 0xA1;
      }
      break;
    case CM_P2DD:
      if(!ServerRW) {
        ...
        DataSvrStep = DATA_SVR_RECIEVE_WAIT;
        memset(&SysCtl,0,sizeof(stSysControl));
        memcpy(&SysCtl, Gpck.Data, sizeof(stSysControl));
        ServerRW = 0xB1;
      }
      break;      
    case CM_MDIR:  
...
{
  unsigned long dlen, i, data_cnt, size, data_ok_cnt, temp;   
  
  DataSvrWaitCount = 0;
  
  if(mode == 0) {
    Setting.ECU_Ver[0] = ReadValue.Ver/1000; temp = ReadValue.Ver%1000;
    Setting.ECU_Ver[1] = temp/100; temp = temp%100;
    Setting.ECU_Ver[2] = temp/10;  temp = temp%10;
    Setting.ECU_Ver[3] = temp;
