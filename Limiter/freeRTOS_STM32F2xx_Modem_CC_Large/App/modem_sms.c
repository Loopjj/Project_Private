...
void DecodeSMS(u8 msg_delete)
{
  ...
  if(SMSData.Msg[0] == '&')  {
    ...
    else if(IsSameString2(SMSData.Msg, "#SETSVRINTERVAL ", SMSData.MCnt, 16, &start)){
        cmd = 21;
    }
    else if(IsSameString2(SMSData.Msg, "#SETRVFACTOR ", SMSData.MCnt, 13, &start)){
        cmd = 22;
    }
    else if(IsSameString2(SMSData.Msg, "#SETVEHINFO ", SMSData.MCnt, 12, &start)){
        cmd = 23;
    }
    else if(IsSameString2(SMSData.Msg, "#LOOPMODE", SMSData.MCnt, 9, &start)){
        cmd = 100;
    }
  ...
  case 210:   //#REQSTATUS
      if(msg_delete == 1) {
        memset(RespMsg,0,80);
        sprintf(RespMsg, "STATUS %u,%u,%u", ReadValue.Sig.KeyOn, ModemStep, DataServerFlag);
        Modem_SMSSend(SMSData.RcvNum,RespMsg, StringLen(RespMsg,80));
      }
      else if(msg_delete == 0) {RxSmsCnt++; memcpy(RcvNum, SMSData.RcvNum, sizeof(RcvNum));}
      break;      
