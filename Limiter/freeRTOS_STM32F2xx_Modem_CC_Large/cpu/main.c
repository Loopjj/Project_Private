...
static void prvTimerCallbackFND(xTimerHandle xTimer);
 ...
 int main(void)
{
  ...
  framSemaphore_Init();
	...
	/* Task Init */
	xTaskCreate( Task_CLI, 	(signed portCHAR*)CLITASK_NAME,	configMINIMAL_STACK_SIZE*2,	(void *)&_sysinfo,	CLI_TASK_PRIO,	NULL);
	xTaskCreate( TmrTask, 	(signed portCHAR*)TMRTASK_NAME,	configMINIMAL_STACK_SIZE*2,	(void *)&_sysinfo,	TMR_TASK_PRIO,	NULL);
  xTaskCreate( Task_MOD,  (signed portCHAR*)MODTASK_NAME,	configMINIMAL_STACK_SIZE*4, (void *)&_sysinfo,	MOD_TASK_PRIO,	NULL);
	xTaskCreate( Task_CAN,  (signed portCHAR*)CANTASK_NAME,	configMINIMAL_STACK_SIZE*2, (void *)&_sysinfo,	CAN_TASK_PRIO,	&xHandle_can);
  xTaskCreate( Task_OBD,  (signed portCHAR*)OBDTASK_NAME,	configMINIMAL_STACK_SIZE*2, (void *)&_sysinfo,	OBD_TASK_PRIO,	NULL);
  xTaskCreate( Task_GPS,  (signed portCHAR*)GPSTASK_NAME,	configMINIMAL_STACK_SIZE*2, (void *)&_sysinfo,	GPS_TASK_PRIO,	NULL);
	xTaskCreate( Task_KEY,  (signed portCHAR*)KEYTASK_NAME,	configMINIMAL_STACK_SIZE*2, (void *)&_sysinfo,	KEY_TASK_PRIO,	NULL);
  
  vTaskSuspend(xHandle_can); // CAN Task stop
  
	/* Timer Calback */
	xTimer01     = xTimerCreate( ( const signed char * ) "led",( 100 / portTICK_RATE_MS ),pdTRUE,( void * ) 0, prvTimerCallback);
	if(xTimer01 != NULL) xTimerStart(xTimer01,0);
	xTimer02     = xTimerCreate( ( const signed char * ) "time",( 1000 / portTICK_RATE_MS ),pdTRUE,( void * ) 0, prvTimerCallback1000msec);
	...
	xTimer04     = xTimerCreate( ( const signed char * ) "fnd",( 2 / portTICK_RATE_MS ),pdTRUE,( void * ) 0, prvTimerCallbackFND);
...
static void prvTimerCallback1000msec(xTimerHandle xTimer)
{
  if(Flags.CANWaitFlag == 1)SendSaveFlag();
  Flags.KeyOnFlag = 1;
  Flags.IntFlag = 1;
  KeyOn_Check();
  ...
  //TANK LEVEL 카운터
	...
	if(Flags1.QualityCntFlag) QualityCnt++;
	else QualityCnt = 0;
...
/* 2mSec 간격으로 Callback Function				                           *
** 출력 데이터를 FND에 Display                                       *
** 시간을 짧게 할수록 FND 밝기를 진하게 함.                          *
*/
static void prvTimerCallbackFND(xTimerHandle xTimer)
{
