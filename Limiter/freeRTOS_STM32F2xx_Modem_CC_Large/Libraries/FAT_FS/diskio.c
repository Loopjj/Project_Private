...
__weak DWORD get_fattime(void) {
  sysinfo_t *s = &_sysinfo 	;
	...
	return 	((DWORD)(s->tm_ptr.tm_year-1900) << 25)
				| ((DWORD)s->tm_ptr.tm_mon << 21)
				| ((DWORD)s->tm_ptr.tm_mday << 16)
				| ((DWORD)s->tm_ptr.tm_hour << 11)
				| ((DWORD)s->tm_ptr.tm_min << 5)
				| ((DWORD)s->tm_ptr.tm_sec >> 1);
}
