...
{
	TmrStop(TMR_USBINTERVAL);
}
void TM_DELAY_SetTime(uint32_t interval)
{
	TmrStop(TMR_USBINTERVAL);
	TmrSetT(TMR_USBINTERVAL, interval);
	TmrStart(TMR_USBINTERVAL);
}
uint32_t TM_DELAY_Time(void)
{
	return TmrChk(TMR_USBINTERVAL);
}
...
USBH_Status USBH_HandleControl (USB_OTG_CORE_HANDLE *pdev, USBH_HOST *phost)
{
  ...
  case CTRL_SETUP_WAIT:
    ...
    if(URB_Status != URB_IDLE) USB_RespWaitCount = 0;
    else if(USB_RespWaitCount > 100) {  // 1 sec
      phost->gState = HOST_ERROR_STATE;
      USB_RespWaitCount = 0;
    }      
