...
*/       
u32 Prev_rmctime;
u32 GPS_SyncOKTime, GPS_SyncNoTime;
void CheckGPSTimeSyc(void)
{
  ...
  if(GPS_Valid && rmc.time != Prev_rmctime) {
    ...
    if(RTC_GPS_Val > LOW_LIMT_OF_TIME) {
      if(diff > 3) {
        if(GPS_SyncOKTime < 60 && GPS_SyncNoTime > 60) {
          GPS_Time_Sync = 1; 
          if(diff > 86400)   UpdateTimeToGPSTime();   //하루이상 시간차일때 바로 업데이트 하고 그렇지 않으면 키오프시 업데이트..
        }
        GPS_SyncNoTime++;
      }
      else {
...
{ 
  ...
  if(Setting.Can_Enable && (CAN_Enable.Speed || CAN_Enable.Speed2))
    speed = CAN_Val.Speed;
  ...
  if (Setting.Can_Enable  && (CAN_Enable.ODO || CAN_Enable.ODO2)){
    ...
    if(!Power_Off_Mode) {          //Key-Off를 감시함
      ...
      if(Setting.Can_Enable) {  //CAN입력시 
        ...
        if(Setting.FW_Type == MDT_S1) {
          ...
          POWER_ENABLE_OFF();         // 통신환경이 없을 경우 바로 꺼짐
