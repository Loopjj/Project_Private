...
void DecodeSMS(u8 msg_delete)
{
      ...
      if(val_item == 4) {
        ...
        if(answer_flag == 1) {
          if(start == 4) {
            ...
            memset(RespMsg,0,80);
            kidx = 0;
            val_cnt = StringLen((char*)VRN,12);
            memcpy(&RespMsg[kidx], VRN, val_cnt);
            kidx += val_cnt;
            RespMsg[kidx] =','; kidx++;
            for(i = 0; i < 17; i++) {
  ...
  case 210:   //#REQSTATUS
      if(msg_delete == 1) {
        memset(RespMsg,0,80);
        sprintf(RespMsg, "STATUS %u,%u,%u", Power_Off_Mode, ModemStep, DataServerFlag);
        Modem_SMSSend(SMSData.RcvNum,RespMsg, StringLen(RespMsg,80));
      }
      else if(msg_delete == 0) {RxSmsCnt++; memcpy(RcvNum, SMSData.RcvNum, sizeof(RcvNum));}
      break;     
