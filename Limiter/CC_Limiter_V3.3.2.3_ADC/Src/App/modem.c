...
void TxStrToModem(char* str) //modem send 
{
  u8 len;
...
******************************************************************************** 
*/
unsigned char Modem_PowerOnCheck(void) //232
{
  u8 resp=0, sendcnt=0;
...
unsigned char Modem_CREG_Check(void)
{
  ...
  if(val2 == 1 || val2 == 5) {
    resp = 1;
    SecData.SysCode.Comm = 0;
  }
  else {
    printk("\r\nMODEM : CREG ERROR\r\n");   
    SecData.SysCode.Comm = 1;
    resp = 0;
  }
...
//  unsigned char len;
  ...
  if(resp == 1 && ATCmdRxMsg[0][0] !=  0) {
    ...
    else if(ModemVer[0] == 'T' && (ModemVer[2] == '4' || ModemVer[2] == '5' || ModemVer[2] == '7'))
...
//  put_ch_console(r);
  ...
  if(r == '\r' || r == '\n') {
    ...
    if(!Urc.RespFlag || Modem_FotaFlag) {
      ...
      else if (strstr(Urc.Buff, "$$NOTI:34") != NULL || strstr(Urc.Buff, "$$TELL:34") != NULL || strstr(Urc.Buff, "$$A_BOOT_ALERT") != NULL)  
      {
        Setting.ExtModemModel = 1;
        TldState = 1;   //Boot up
        ModemStep = MODEM_POWER_ON_IDLE;  
...
void Modem_Parameter_Reset(void)
{
  ...
  sprintf(HostSvr.SvrName,"lb.aea-rm.com");
  HostSvr.SvrPort = 3001;
  FRAMMultiWrite((u8 *)&HostSvr, DATA_SERVER_INFO, sizeof(tsSvrInfo));

  ip[0] = 14; ip[1] = 63; ip[2] = 200; ip[3] = 14;      // Loop server
