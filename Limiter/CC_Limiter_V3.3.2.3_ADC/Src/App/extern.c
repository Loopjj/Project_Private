...
void CarRegNoToVrn(void) {
  ...
  if(Setting.Car_Reg_No[1] > 0 && Setting.Car_Reg_No[1] < MAX_CAR_REGION) {
    ...
    idx = 0;
    if(Setting.Car_Reg_No[1] > '0' && Setting.Car_Reg_No[1] <= '9')
      VRN[idx++] = Setting.Car_Reg_No[1];
    else Setting.Car_Reg_No[1] = 0;
    VRN[idx++] = Setting.Car_Reg_No[2]+0x30;
    VRN[idx++] = Setting.Car_Reg_No[3]+0x30;
    VRN[idx++] = Car_No_Class[Setting.Car_Reg_No[4]][0];
    VRN[idx++] = Car_No_Class[Setting.Car_Reg_No[4]][1];
    VRN[idx++] = Setting.Car_Reg_No[5]+0x30;
    VRN[idx++] = Setting.Car_Reg_No[6]+0x30;
    VRN[idx++] = Setting.Car_Reg_No[7]+0x30;
    VRN[idx++] = Setting.Car_Reg_No[8]+0x30;  
    while(idx < 12)  VRN[idx++] = 0;  
  }
}    

/*
********************************************************************************
* Description : VrnToCarRegNo
* Arguments   : u8 *rx_reg_no, u8 len
* Return      : 
* Note        : 
******************************************************************************** 
*/
u8 VrnToCarRegNo(u8 *rx_reg_no, u8 len) {
  u8 i, idx, kidx, ridx, val_cnt = 0, car_reg_no[12], reg_no[12];
  u16 addr;
  
  memset(car_reg_no,0,12);
  memset(reg_no,0,12);
  for(i = 0; i < len; i++)     if(rx_reg_no[i] != '#' && rx_reg_no[i] != ' '&& rx_reg_no[i] != 0) reg_no[val_cnt++] = rx_reg_no[i];
  ...
  if(reg_no[0] > 0) {
    if((reg_no[0] >= '0' && reg_no[0] <= '9')  && (len == 7 || len == 8  || len == 9)) {
      if(!(reg_no[1] >= '0' && reg_no[1] <= '9') && len == 7) {
        car_reg_no[1] = 0; 
        car_reg_no[2] = 0;
        car_reg_no[3] = reg_no[1]-0x30;
        idx = 1;
      }
      else if((reg_no[1] >= '0' && reg_no[1] <= '9') && (reg_no[2] >= '0' && reg_no[2] <= '9') && !(reg_no[3] >= '0' && reg_no[3] <= '9') && len == 9) {
        if(reg_no[0] == '0') car_reg_no[1] = 0;
        else car_reg_no[1] = reg_no[0]; 
        car_reg_no[2] = reg_no[1]-0x30;
        car_reg_no[3] = reg_no[2]-0x30;
        idx = 3;
      }      
      else if((reg_no[1] >= '0' && reg_no[1] <= '9') && !(reg_no[2] >= '0' && reg_no[2] <= '9') && len == 8) {
        car_reg_no[1] = 0; 
        car_reg_no[2] = reg_no[0]-0x30; 
        car_reg_no[3] = reg_no[1]-0x30;
        idx = 2;
      }
      else return 0;
      ...
      for(i = 0; i < sizeof(Car_No_Class)/2; i++) {
        if(reg_no[idx] == Car_No_Class[i][0] && reg_no[idx+1] == Car_No_Class[i][1]) {
          kidx = i;
          val_cnt++;
          break;
        }
      ...
      idx = idx+2;
      for(i = 0; i < 4; i++)
        if(reg_no[idx+i] >= '0' && reg_no[idx+i] <= '9')
          val_cnt++;
      if(val_cnt == 5) {
        car_reg_no[4] = kidx;  
        car_reg_no[5] = reg_no[idx]-0x30;  
        car_reg_no[6] = reg_no[idx+1]-0x30;  
        car_reg_no[7] = reg_no[idx+2]-0x30;  
        car_reg_no[8] = reg_no[idx+3]-0x30;
      }
      else return 0;
    }
    else {   //지역번호 있을때
      ...
      for(i = 0; i < sizeof(Car_No_Region)/4; i++) {
        if(IsSameString((char*)&reg_no[0], (char*)Car_No_Region[i],4)) {
          val_cnt++;
          ridx = i;
          break;
        }
      ...
      for(i = 0; i < 2; i++) {
        if(reg_no[4+i] >= '0' && reg_no[4+i] <= '9')
          val_cnt++;
      }
      if(val_cnt < 3) return 0;
      
      for(i = 0; i < sizeof(Car_No_Class)/2; i++) {
        if(reg_no[6] == Car_No_Class[i][0] && reg_no[7] == Car_No_Class[i][1]) {
          kidx = i;
          val_cnt++;
          break;
        }
      }
      if(val_cnt < 4) return 0;

      for(i = 0; i < 4; i++)
        if(reg_no[8+i] >= '0' && reg_no[8+i] <= '9')
          val_cnt++;

      if(val_cnt == 8) {
        car_reg_no[0] = 0;
        car_reg_no[1] = ridx; 
        car_reg_no[2] = reg_no[4]-0x30;  
        car_reg_no[3] = reg_no[5]-0x30;  
        car_reg_no[4] = kidx;  
        car_reg_no[5] = reg_no[8]-0x30;  
        car_reg_no[6] = reg_no[9]-0x30;  
        car_reg_no[7] = reg_no[10]-0x30;  
        car_reg_no[8] = reg_no[11]-0x30;
      }
      else return 0;
    }
    memset(VRN, 0, sizeof(VRN));
    memcpy(VRN, reg_no, len);
    memcpy(&Setting.Car_Reg_No[1], &car_reg_no[1], sizeof(Setting.Car_Reg_No)-1);
    addr = DRV_SETTING_PARM + (u8 *)&Setting.Car_Reg_No - (u8 *)&Setting;
    FRAMMultiWrite((u8 *)&Setting.Car_Reg_No, addr, sizeof(Setting.Car_Reg_No)); 
    return 1;
  }
  else return 0;
}           
