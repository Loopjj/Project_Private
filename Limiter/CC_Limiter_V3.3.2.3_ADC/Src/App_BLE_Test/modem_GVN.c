...
{
  u8 len, resp = 0, i, kidx, ridx, val_cnt, car_reg_no[12];
  u32 val; 
  ...
  switch(CLI_Cmd) {
    ...
    case 7:  //VRN
      ...
      if(CLI_RxMsg[0] > 0) {
        if((CLI_RxMsg[0] >= '0' && CLI_RxMsg[0] <= '9')  && (CLI_RxMsg[1] >= '0' && CLI_RxMsg[1] <= '9')){
          val_cnt = 0;                              //가나다...
          for(i = 0; i < sizeof(Car_No_Class)/2; i++) {
            if(CLI_RxMsg[2] == Car_No_Class[i][0] && CLI_RxMsg[3] == Car_No_Class[i][1]) {
              kidx = i;
              val_cnt++;
              break;
            }
          ...
          for(i = 0; i < 4; i++)
            if(CLI_RxMsg[4+i] >= '0' && CLI_RxMsg[4+i] <= '9')
              val_cnt++;
          if(val_cnt == 5) {
            car_reg_no[0] = 0;
            car_reg_no[1] = 0; 
            car_reg_no[2] = CLI_RxMsg[0]-0x30;  
            car_reg_no[3] = CLI_RxMsg[1]-0x30;  
            car_reg_no[4] = kidx;  
            car_reg_no[5] = CLI_RxMsg[4]-0x30;  
            car_reg_no[6] = CLI_RxMsg[5]-0x30;  
            car_reg_no[7] = CLI_RxMsg[6]-0x30;  
            car_reg_no[8] = CLI_RxMsg[7]-0x30;
          }
        }
        else {   //지역번호 있을때
                                  //지역번호
          ...
          for(i = 0; i < sizeof(Car_No_Region)/4; i++) {
            if(IsSameString((char*)&CLI_RxMsg[0], (char*)Car_No_Region[i],4)) {
              val_cnt++;
              ridx = i;
              break;
            }
          ...
          for(i = 0; i < sizeof(Car_No_Class)/2; i++) {
            if(CLI_RxMsg[6] == Car_No_Class[i][0] && CLI_RxMsg[7] == Car_No_Class[i][1]) {
              kidx = i;
              val_cnt++;
              break;
            }
                                       ...
                                       //car no.2
          for(i = 0; i < 4; i++)
            if(CLI_RxMsg[8+i] >= '0' && CLI_RxMsg[8+i] <= '9')
              val_cnt++;

          if(val_cnt == 8) {
            car_reg_no[0] = 0;
            car_reg_no[1] = ridx; 
            car_reg_no[2] = CLI_RxMsg[4]-0x30;  
            car_reg_no[3] = CLI_RxMsg[5]-0x30;  
            car_reg_no[4] = kidx;  
            car_reg_no[5] = CLI_RxMsg[8]-0x30;  
            car_reg_no[6] = CLI_RxMsg[9]-0x30;  
            car_reg_no[7] = CLI_RxMsg[10]-0x30;  
            car_reg_no[8] = CLI_RxMsg[11]-0x30;
          }
        }
      }      
      memset(VRN, 0, sizeof(VRN));
      memcpy(VRN, CLI_RxMsg, len);
