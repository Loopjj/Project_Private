...
/*
********************************************************************************
* Description : VRN_Conversion
* Arguments   : reg_no
* Return      : 
* Note        : VRN Conversion
******************************************************************************** 
*/
void VRN_Conversion(u8 *reg_no, u8 len)
{      
  u8 i, kidx, ridx, val_cnt, car_reg_no[12];
  u16 addr;
  
  memset(car_reg_no,0,12);
  if(reg_no[0] > 0) {
    if((reg_no[0] >= '0' && reg_no[0] <= '9')  && (reg_no[1] >= '0' && reg_no[1] <= '9')){
      val_cnt = 0;                              //가나다...
      for(i = 0; i < sizeof(Car_No_Class)/2; i++) {
        if(reg_no[2] == Car_No_Class[i][0] && reg_no[3] == Car_No_Class[i][1]) {
          kidx = i;
          val_cnt++;
          break;
        }
      ...
      if(val_cnt == 5) {
        ...
        car_reg_no[4] = kidx;  
        car_reg_no[5] = reg_no[4]-0x30;  
        car_reg_no[6] = reg_no[5]-0x30;  
        car_reg_no[7] = reg_no[6]-0x30;  
        car_reg_no[8] = reg_no[7]-0x30;
      }
    }
    else {   //지역번호 있을때
                              //지역번호
  ...
  if(mode == 0) {
    ...
    if(ReceiveMediaFlag == BLESCANNER) {
      if(TxBLE_Index == 0) {        // BLE Event
        ...
        BLE_Send_Data(0x73, 0x12, 0x01, ModemBuff, dlen);
        BLE_SmartWaitFlag = 1;
      }
    }
    else {
      if(TxRFID_Index == 0) {       // RFID Card Event
        BleServerFlag = 0;
      }
      else {
