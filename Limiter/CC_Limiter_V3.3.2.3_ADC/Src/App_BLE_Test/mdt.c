...
u8 Make_EventDataLG(u8 step, u8* ev_data, u16 ccrc)
{
  ...
  for(i = 0; i < ev_data_cnt[step]; i++) {
    ...
    if(step == 0 && i == 0) {
      memcpy(&ev_data[idx], stemp.Time, 7);
      idx += 7;
    }
    ...
    ev_data[idx] = stemp.G_X; idx++;
    ev_data[idx] = stemp.G_Y; idx++;
    ev_data[idx] = 1; idx++;
    ev_data[idx] = 0; idx++;
    ev_data[idx] = 0; idx++;
  }
...
//          memcpy(&sms.CarType,MDT_RxBuf[MDT_DecodeIndex],MDT_RxLength[MDT_DecodeIndex]);
          ...
          if(IsDigitBuff(sms->CarRegNo,sizeof(sms->CarRegNo))) {      
            ...
            if(r[0] < 19 && r[1] < 51) {//r[0]=18 세종 , r[1]=50 호
...
u8* CarNo_BinToAscii_S1(u8 *data)
{
  ...
  if(Setting.FW_Type != MDT_LG) {
    if(data[1] == 0) { //car region == blank
      ret[0]  = data[2]+0x30;
      ret[1]  = data[3]+0x30;
      ret[2]  = Car_No_Class[clss][0];
      ret[3]  = Car_No_Class[clss][1];
      ret[4]  = data[5]+0x30;
      ret[5]  = data[6]+0x30;
      ret[6]  = data[7]+0x30;
      ret[7]  = data[8]+0x30;
    ...
    else {  //car region == 서울,부산, etc...
      ret[0] = Car_No_Region[regn][0];
      ret[1] = Car_No_Region[regn][1];
      ret[2] = Car_No_Region[regn][2];
      ret[3] = Car_No_Region[regn][3];
      ret[4] = data[2]+0x30;
      ret[5] = data[3]+0x30;
      ret[6] = Car_No_Class[clss][0];
      ret[7] = Car_No_Class[clss][1];
      ret[8] = data[5]+0x30;
      ret[9] = data[6]+0x30;
      ret[10] = data[7]+0x30;
      ret[11] = data[8]+0x30;
